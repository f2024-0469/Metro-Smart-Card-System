...
Run make clean && make and confirm no warnings.
Run both menu mode and --script mode.
Test using the full data files.
Valgrind has been run under Ubuntu WSL with no memory leaks or errors; retain the clean report in the validation documentation.
Keep the measured-vs-claimed table with your final report.
Ensure the GitHub repository has at least 15 meaningful commits spread across the semester.
Both team members should be able to explain the entire codebase during the viva.

 Academic Honesty

This project was developed by the team in accordance with the course academic honesty policy. AI assistance was used only for guidance with Git commands, Valgrind setup and execution, and Ubuntu/WSL terminal commands during project validation and repository management. The team is responsible for understanding and explaining the submitted implementation during the viva.